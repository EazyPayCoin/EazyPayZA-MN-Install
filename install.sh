...
EZPAY Masternode installer arguments:
...
echo "
 ...
 | the EZPAY Team CANNOT assist you in repairing  |::
...
echo "" && echo 'Added user "eazypayza"' && echo ""
sleep 1
...
# Install EZPAY daemon
#wget $TARBALLURL
#tar -xzvf $TARBALLNAME 
#rm $TARBALLNAME
mv ./eazypayzad /usr/local/bin
mv ./eazypayza-cli /usr/local/bin
mv ./eazypayza-tx /usr/local/bin
rm -rf $TARBALLNAME

# Create .eazypayza directory
mkdir $USERHOME/.eazypayza
...
# Create eazypayza.conf
touch $USERHOME/.eazypayza/eazypayza.conf
cat > $USERHOME/.eazypayza/eazypayza.conf << EOL
${INSTALLERUSED}
rpcuser=${RPCUSER}
...
masternode=1
addnode=173.208.215.11
addnode=173.208.215.12
addnode=173.208.215.13
addnode=173.208.215.14
EOL
chmod 0600 $USERHOME/.eazypayza/eazypayza.conf
chown -R $USER:$USER $USERHOME/.eazypayza

sleep 1

cat > /etc/systemd/system/eazypayza.service << EOL
[Unit]
...
ExecStart=/usr/local/bin/eazypayzad -conf=${USERHOME}/.eazypayza/eazypayza.conf -datadir=${USERHOME}/.eazypayza
ExecStop=/usr/local/bin/eazypayza-cli -conf=${USERHOME}/.eazypayza/eazypayza.conf -datadir=${USERHOME}/.eazypayza stop
Restart=on-abort
[Install]
WantedBy=multi-user.target
EOL
sudo systemctl enable eazypayza.service
sudo systemctl start eazypayza.service
