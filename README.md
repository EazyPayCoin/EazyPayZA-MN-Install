...
* First, we will do the initial collateral TX and send exactly 5000 EZPAY to one of our addresses. To keep things sorted in case we setup more masternodes we will label the addresses we use.

  - Open your EZPAY wallet and switch to the "Receive" tab.
  ...
  - Once all addresses are created send 5000 EZPAY each to them. Ensure that you send exactly 5000 EZPAY and do it in a single transaction. You can double check where the coins are coming from by checking it via coin control usually, that's not an issue.
...
Close your wallet and open the EazyPayZA Appdata folder. Its location depends on your OS.

* **Windows:** Press Windows+R and write %appdata% - there, open the folder EazyPayZA.  
* **macOS:** Press Command+Space to open Spotlight, write ~/Library/Application Support/EazyPayZA and press Enter.  
* **Linux:** Open ~/.eazypayza/

In your appdata folder, open masternode.conf with a text editor and add a new line in this format to the bottom of the file:

```bash
masternodename ipaddress:5566 genkey collateralTxID outputID
```

An example would be

```
mn1 127.0.0.2:5566 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0
```
...
```bash
bash <( curl https://raw.githubusercontent.com/EazyPayCoin/EazyPayZA-MN-Install/master/install.sh )
```
...
**PLEASE NOTE**: Do not choose the advanced installation option unless you have experience with Linux and know what you are doing - if you do and something goes wrong, the EazyPayZA team CANNOT help you, and you will have to restart the installation.
...
```bash
cd /usr/local/bin
./eazypayza-cli masternode status
```



## For Windows setups, use this config in your masternode.conf or eazypayza.conf (depending on if you are using a VPS or local wallet)
